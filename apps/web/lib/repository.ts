...
export async function buildMindGraphSnapshot(userId: string): Promise<MindGraphSnapshot> {
  const [mindNodes, mindEdges, dockItems, tags, entryTagRelations, entryRelations, recommendations, entries] = await Promise.all([
    mindNodesTable.where('userId').equals(userId).toArray(),
    mindEdgesTable.where('userId').equals(userId).toArray(),
    dockItemsTable.where('userId').equals(userId).toArray(),
    tagsTable.where('userId').equals(userId).toArray(),
    entryTagRelationsTable.where('userId').equals(userId).toArray(),
    entryRelationsTable.where('userId').equals(userId).toArray(),
    recommendationsTable.where('userId').equals(userId).and(r => r.status === 'generated' || r.status === 'shown' || r.status === 'accepted').toArray(),
    entriesTable.where('userId').equals(userId).toArray(),
  ])

  const nodeMap = new Map<string, MindGraphSnapshotNode>()
  const edgeSet = new Set<string>()
  const snapshotEdges: MindGraphSnapshotEdge[] = []

  const tagIdToRealName = new Map<string, string>()
  tags.forEach(t => {
    if (t.id && t.name) tagIdToRealName.set(t.id, t.name)
  })

  const dockItemById = new Map<number, DockItemRecord>()
  dockItems.forEach(d => {
    if (d.id !== undefined) dockItemById.set(d.id as number, d)
  })

  const entryById = new Map<number, EntryRecord>()
  entries.forEach(e => {
    if (e.id !== undefined) entryById.set(e.id as number, e)
  })

  mindNodes.forEach(n => {
    const persisted = toPersistedMindNode(n)
    if (!persisted) return
    nodeMap.set(persisted.id, {
      id: persisted.id,
      nodeType: persisted.nodeType,
      label: persisted.label,
      documentId: persisted.documentId,
      state: persisted.state,
      degreeScore: persisted.degreeScore,
      recentActivityScore: persisted.recentActivityScore,
      documentWeightScore: persisted.documentWeightScore,
      userPinScore: persisted.userPinScore,
      clusterCenterScore: persisted.clusterCenterScore,
      positionX: persisted.positionX,
      positionY: persisted.positionY,
      metadata: persisted.metadata,
    })
  })

  entries.forEach(entry => {
    const existingNodeId = Array.from(nodeMap.entries()).find(([, v]) => v.documentId === entry.id)?.[0]
    if (existingNodeId) return
    const nodeId = makeMindNodeId(userId, 'document', entry.title || `entry_${entry.id}`)
    if (nodeMap.has(nodeId)) return
    const pos = seededPosition(nodeId, nodeMap.size, nodeMap.size + entries.length + dockItems.length)
    nodeMap.set(nodeId, {
      id: nodeId,
      nodeType: 'document',
      label: entry.title || `Entry #${entry.id}`,
      documentId: entry.id as number,
      state: 'drifting',
      degreeScore: 1,
      ...
      positionX: pos.x,
      positionY: pos.y,
      metadata: { source: 'entry_table', project: entry.project, tags: entry.tags },
    })
    if (entry.project) {
      const projectId = makeMindNodeId(userId, 'project', entry.project)
      if (!nodeMap.has(projectId)) {
        const pp = seededPosition(projectId, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(projectId, {
          id: projectId, nodeType: 'project', label: entry.project, documentId: null, state: 'active',
          degreeScore: 1, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0,
          positionX: pp.x, positionY: pp.y, metadata: { source: 'entry_project' },
        })
      }
      const ek1 = `${projectId}::${nodeId}::parent_child`
      if (!edgeSet.has(ek1)) { edgeSet.add(ek1); snapshotEdges.push({ id: `snap_${ek1}`, sourceNodeId: projectId, targetNodeId: nodeId, edgeType: 'parent_child', strength: 0.7, source: 'system', confidence: 0.7, reason: 'entry project' }) }
    }
    entry.tags?.forEach((tagName: string) => {
      if (!tagName) return
      const tid = makeMindNodeId(userId, 'tag', tagName)
      if (!nodeMap.has(tid)) {
        const tp = seededPosition(tid, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(tid, { id: tid, nodeType: 'tag', label: tagName, documentId: null, state: 'drifting', degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0, positionX: tp.x, positionY: tp.y, metadata: { source: 'entry_tags' } })
      }
      const ek2 = `${nodeId}::${tid}::semantic`
      if (!edgeSet.has(ek2)) { edgeSet.add(ek2); snapshotEdges.push({ id: `snap_${ek2}`, sourceNodeId: nodeId, targetNodeId: tid, edgeType: 'semantic', strength: 0.5, source: 'system', confidence: 0.5, reason: 'entry tag' }) }
    })
  })

  dockItems.forEach(d => {
    if (d.id === undefined) return
    const existingNodeId = Array.from(nodeMap.entries()).find(([, v]) => v.documentId === d.id)?.[0]
    if (existingNodeId) return
    const label = d.topic || d.rawText.slice(0, 50)
    const nodeId = makeMindNodeId(userId, 'document', label)
    if (nodeMap.has(nodeId)) return
    const pos = seededPosition(nodeId, nodeMap.size, nodeMap.size + 20)
    nodeMap.set(nodeId, {
      id: nodeId, nodeType: 'document', label, documentId: d.id as number, state: 'drifting',
      degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0,
      positionX: pos.x, positionY: pos.y, metadata: { source: 'dock_item', project: d.selectedProject, tags: d.userTags },
    })
    if (d.selectedProject) {
      const pid = makeMindNodeId(userId, 'project', d.selectedProject)
      if (!nodeMap.has(pid)) {
        const pp = seededPosition(pid, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(pid, { id: pid, nodeType: 'project', label: d.selectedProject, documentId: null, state: 'active', degreeScore: 1, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0, positionX: pp.x, positionY: pp.y, metadata: { source: 'dock_project' } })
      }
      const ek = `${pid}::${nodeId}::parent_child`
      if (!edgeSet.has(ek)) { edgeSet.add(ek); snapshotEdges.push({ id: `snap_${ek}`, sourceNodeId: pid, targetNodeId: nodeId, edgeType: 'parent_child', strength: 0.7, source: 'system', confidence: 0.7, reason: 'dock project' }) }
    }
    d.userTags?.forEach((tagName: string) => {
      const tid = makeMindNodeId(userId, 'tag', tagName)
      if (!nodeMap.has(tid)) {
        const tp = seededPosition(tid, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(tid, { id: tid, nodeType: 'tag', label: tagName, documentId: null, state: 'drifting', degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0, positionX: tp.x, positionY: tp.y, metadata: { source: 'dock_tags' } })
      }
      const ek = `${nodeId}::${tid}::semantic`
      if (!edgeSet.has(ek)) { edgeSet.add(ek); snapshotEdges.push({ id: `snap_${ek}`, sourceNodeId: nodeId, targetNodeId: tid, edgeType: 'semantic', strength: 0.5, source: 'system', confidence: 0.5, reason: 'dock tag' }) }
    })
    if (d.topic) {
      const qid = makeMindNodeId(userId, 'topic', d.topic)
      if (!nodeMap.has(qid)) {
        const qp = seededPosition(qid, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(qid, { id: qid, nodeType: 'topic', label: d.topic, documentId: null, state: 'drifting', degreeScore: 1, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0, positionX: qp.x, positionY: qp.y, metadata: { source: 'dock_topic' } })
      }
      const ek = `${qid}::${nodeId}::parent_child`
      if (!edgeSet.has(ek)) { edgeSet.add(ek); snapshotEdges.push({ id: `snap_${ek}`, sourceNodeId: qid, targetNodeId: nodeId, edgeType: 'parent_child', strength: 0.6, source: 'system', confidence: 0.6, reason: 'dock topic' }) }
    }
    if (d.sourceId != null && dockItemById.has(d.sourceId)) {
      const srcItem = dockItemById.get(d.sourceId)
      if (!srcItem) return
      const srcLabel = srcItem.topic || srcItem.rawText.slice(0, 50)
      const srcId = makeMindNodeId(userId, 'document', srcLabel)
      if (nodeMap.has(srcId)) {
        const ek = `${srcId}::${nodeId}::reference`
        if (!edgeSet.has(ek)) { edgeSet.add(ek); snapshotEdges.push({ id: `snap_${ek}`, sourceNodeId: srcId, targetNodeId: nodeId, edgeType: 'reference', strength: 0.5, source: 'system', confidence: 0.5, reason: 'dock sourceId ref' }) }
      }
    }
  })
  ...
  tags.forEach(tag => {
    ...
    const nodeId = makeMindNodeId(userId, 'tag', tag.name)
    if (nodeMap.has(nodeId)) return
    const pos = seededPosition(nodeId, nodeMap.size, nodeMap.size + 20)
    nodeMap.set(nodeId, {
      id: nodeId, nodeType: 'tag', label: tag.name, documentId: null, state: 'drifting',
      degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0,
      positionX: pos.x, positionY: pos.y, metadata: { source: 'tag_table' },
    })
  })

  mindEdges.forEach(e => {
    const persisted = toPersistedMindEdge(e)
    if (!persisted) return
    if (!nodeMap.has(persisted.sourceNodeId) || !nodeMap.has(persisted.targetNodeId)) return
    const edgeKey = `${persisted.sourceNodeId}::${persisted.targetNodeId}::${persisted.edgeType}`
  ...
  const documentNodeByDocId = new Map<number, string>()
  nodeMap.forEach((node, nodeId) => {
    if (node.documentId != null) documentNodeByDocId.set(node.documentId, nodeId)
  })

  entryTagRelations.forEach(rel => {
    if (!rel.id || !rel.entryId || !rel.tagId) return
    const realTagName = tagIdToRealName.get(rel.tagId)
    if (!realTagName) return
    const tagNodeId = makeMindNodeId(userId, 'tag', realTagName)
    if (!nodeMap.has(tagNodeId)) {
      const tp = seededPosition(tagNodeId, nodeMap.size, nodeMap.size + 20)
      nodeMap.set(tagNodeId, {
        id: tagNodeId, nodeType: 'tag', label: realTagName, documentId: null, state: 'drifting',
        degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0,
        positionX: tp.x, positionY: tp.y, metadata: { source: 'entry_tag_relation' },
      })
    }
    const docNodeId = documentNodeByDocId.get(rel.entryId)
    if (!docNodeId) return
    const edgeKey = `${docNodeId}::${tagNodeId}::semantic`
    if (edgeSet.has(edgeKey)) return
    edgeSet.add(edgeKey)
    snapshotEdges.push({
      id: `snap_${edgeKey}`,
      sourceNodeId: docNodeId, targetNodeId: tagNodeId,
      edgeType: 'semantic', strength: rel.confidence ?? 0.5, source: 'system',
      confidence: rel.confidence ?? 0.5, reason: `entry-tag relation from ${rel.source}`,
    })
  })

  entryRelations.forEach(rel => {
    if (!rel.id || !rel.sourceEntryId || !rel.targetEntryId) return
    const srcNodeId = documentNodeByDocId.get(rel.sourceEntryId)
    const tgtNodeId = documentNodeByDocId.get(rel.targetEntryId)
    if (!srcNodeId || !tgtNodeId || srcNodeId === tgtNodeId) return
    const edgeType: MindEdgeType = rel.relationType === 'reference' ? 'reference' : 'semantic'
    const edgeKey = `${srcNodeId}::${tgtNodeId}::${edgeType}`
    if (edgeSet.has(edgeKey)) return
    edgeSet.add(edgeKey)
    snapshotEdges.push({
      id: `snap_${edgeKey}`, sourceNodeId: srcNodeId, targetNodeId: tgtNodeId,
      edgeType, strength: rel.confidence ?? 0.5, source: 'system',
      confidence: rel.confidence ?? 0.5, reason: rel.reason ?? `entry relation: ${rel.relationType}`,
    })
  })

  recommendations.forEach(rec => {
    if (!rec.id) return
    const subjectId = typeof rec.subjectId === 'number' ? rec.subjectId : Number(rec.subjectId)
    const candidateId = rec.candidateId
    const candType = rec.candidateType

    if (candType === 'tag') {
      const realName = tagIdToRealName.get(candidateId)
      if (!realName) return
      const tagNodeId = makeMindNodeId(userId, 'tag', realName)
      if (!nodeMap.has(tagNodeId)) {
        const tp = seededPosition(tagNodeId, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(tagNodeId, {
          id: tagNodeId, nodeType: 'tag', label: realName, documentId: null, state: 'drifting',
          degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0,
          positionX: tp.x, positionY: tp.y, metadata: { source: 'recommendation_tag' },
        })
      }
      const docNodeId = documentNodeByDocId.get(subjectId)
      if (!docNodeId) return
      const edgeKey = `${docNodeId}::${tagNodeId}::suggested`
      if (edgeSet.has(edgeKey)) return
      edgeSet.add(edgeKey)
      snapshotEdges.push({ id: `snap_${edgeKey}`, sourceNodeId: docNodeId, targetNodeId: tagNodeId, edgeType: 'suggested', strength: rec.confidenceScore ?? 0.5, source: 'system', confidence: rec.confidenceScore ?? 0.5, reason: `recommendation tag: ${rec.recommendationType}` })
    } else if (candType === 'project' && rec.status === 'accepted') {
      const docNodeId = documentNodeByDocId.get(subjectId)
      if (!docNodeId) return
      const projectNodeId = makeMindNodeId(userId, 'project', candidateId)
      if (!nodeMap.has(projectNodeId)) {
        const pp = seededPosition(projectNodeId, nodeMap.size, nodeMap.size + 20)
        nodeMap.set(projectNodeId, { id: projectNodeId, nodeType: 'project', label: candidateId, documentId: null, state: 'active', degreeScore: 1, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0, positionX: pp.x, positionY: pp.y, metadata: { source: 'recommendation_project' } })
      }
      const edgeKey = `${projectNodeId}::${docNodeId}::confirmed`
      if (edgeSet.has(edgeKey)) return
      edgeSet.add(edgeKey)
      snapshotEdges.push({ id: `snap_${edgeKey}`, sourceNodeId: projectNodeId, targetNodeId: docNodeId, edgeType: 'confirmed', strength: 0.8, source: 'user', confidence: 1.0, reason: `recommendation accepted: ${rec.recommendationType}` })
    } else if (candType === 'mindNode') {
      if (!nodeMap.has(candidateId)) return
      const sourceNodeId = documentNodeByDocId.get(subjectId) || Array.from(nodeMap.keys()).find(nid => nodeMap.get(nid)?.documentId === subjectId)
      if (!sourceNodeId) return
      const edgeType: MindEdgeType = rec.status === 'accepted' ? 'confirmed' : 'suggested'
      const edgeKey = `${sourceNodeId}::${candidateId}::${edgeType}`
      if (edgeSet.has(edgeKey)) return
      edgeSet.add(edgeKey)
      snapshotEdges.push({ id: `snap_${edgeKey}`, sourceNodeId, targetNodeId: candidateId, edgeType, strength: rec.confidenceScore ?? 0.5, source: rec.status === 'accepted' ? 'user' : 'system', confidence: rec.confidenceScore ?? 0.5, reason: `recommendation: ${rec.recommendationType}` })
    }
  })

  let rootNodeId = Array.from(nodeMap.values()).find(n => n.nodeType === 'root')?.id ?? null
  if (!rootNodeId) {
    const rid = makeMindNodeId(userId, 'root', 'World Tree')
    nodeMap.set(rid, {
      id: rid, nodeType: 'root', label: 'World Tree', documentId: null, state: 'active',
      degreeScore: 0, recentActivityScore: 0, documentWeightScore: 0, userPinScore: 0, clusterCenterScore: 0,
      positionX: 0, positionY: 0, metadata: null,
    })
    rootNodeId = rid
  }

  const connectedNodeIds = new Set<string>()
  snapshotEdges.forEach(e => { connectedNodeIds.add(e.sourceNodeId); connectedNodeIds.add(e.targetNodeId) })
  if (rootNodeId) {
    nodeMap.forEach((_node, nid) => {
      if (nid === rootNodeId || connectedNodeIds.has(nid)) return
      const ek = `${rootNodeId}::${nid}::semantic`
      if (!edgeSet.has(ek)) {
        edgeSet.add(ek)
        snapshotEdges.push({ id: `snap_${ek}`, sourceNodeId: rootNodeId as string, targetNodeId: nid, edgeType: 'semantic', strength: 0.1, source: 'system', confidence: 0.1, reason: 'orphan anchor' })
      }
    })
  }
...
export async function updateMindNodePositions(
  userId: string,
  updates: Array<{ nodeId: string; positionX: number; positionY: number }>,
): Promise<void> {
  const now = new Date()
  for (const { nodeId, positionX, positionY } of updates) {
    const existing = await mindNodesTable.get(nodeId)
    if (!existing || existing.userId !== userId) continue
    await mindNodesTable.update(nodeId, {
      positionX,
      positionY,
      updatedAt: now,
    })
